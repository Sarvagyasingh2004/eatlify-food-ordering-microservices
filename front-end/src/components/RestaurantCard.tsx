    ...
    return (
        <div className={`cursor-pointer overflow-hidden rounded-xl bg-white shadow-sm transition hover:shadow-md ${isOpen ? "" : "opacity-80"}`} onClick={() => navigate(`/restaurant/${id}`)}>
            <div className="relative h-40 w-full overflow-hidden">
                <img src={image} alt="RestaurantImage" className={`h-full w-full object-cover transition duration-300 hover:scale-105 ${isOpen ? "" : "grayscale"}`} />
                {
            ...
            <div className="p-3 space-y-1">
                ...
                <p className="tetx-sm text-gray-500">{distance} KM away</p>
            </div>
        </div>
    )
}
